...
- ~~**Find-by-User typeahead in adhoc mode.**~~ ✅ Implemented — `useAadUserSearch` now branches on `_adhocMode` and falls back to `Office365UsersService.SearchUserV2` when the caller lacks `prvReadaaduser`. Inherits the existing 300ms debounce + 2-char min from `UserSearch.tsx`. Boot probe in `App.tsx` flips the adhoc flag proactively (rowCount===0 on aadusers top:1 query → adhoc).
